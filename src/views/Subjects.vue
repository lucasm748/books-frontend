<template>
  <v-container>
    <DefaultDataTable :headers="headers" :items="subjects" @edit="editSubject" @remove="confirmRemoveSubject"
      :card-title="'Assuntos Cadastrados'" />
    <v-btn class="mt-4" color="blue" dark @click="openNewSubjectModal">Novo Assunto</v-btn>
    <v-btn class="mt-4 ml-2" color="green" dark @click="emitReport">Emitir Relatório</v-btn>
    <EditSubjectModal :isVisible="isEditModalVisible" :subject="selectedSubject" :mode="modalMode"
    ...
    <v-dialog v-model="isConfirmDialogVisible" max-width="500px">
      <v-card>
        ...
        <v-card-text>Tem certeza de que deseja excluir este assunto?</v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="blue darken-1" variant="text" @click="isConfirmDialogVisible = false">Cancelar</v-btn>
          <v-btn color="red darken-1" variant="text" @click="removeSubject">Excluir</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
...
async function updateSubject(updatedSubject: Subject) {
  try {
    const response = await axios.put(import.meta.env.VITE_API_URL + `/subjects/${updatedSubject.id}`, updatedSubject);
    const index = subjects.value.findIndex(subject => subject.id === updatedSubject.id);
    if (index !== -1) {
...
async function removeSubject() {
  try {
    await axios.delete(import.meta.env.VITE_API_URL + `/subjects/${selectedSubject.value.id}`);
